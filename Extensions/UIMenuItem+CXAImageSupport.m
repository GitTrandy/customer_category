...
static void newDrawTextInRect(UILabel *self, SEL _cmd, CGRect rect)
{
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0"))
    {
        ...
        if ([superv isKindOfClass:NSClassFromString(@"UICalloutBarButton")])
        {
            if ([self.text isEqualToString:MultiLanguage(cvcCopy)]) {
                superv.backgroundColor = [UIColor colorWithRed:46.f/255.f green:46.f/255.f blue:46.f/255.f alpha:1.f];
                [superv setRoundedCorners:(UIRectCornerTopLeft | UIRectCornerBottomLeft) radius:5.f];
                
                CGRect f = superv.frame;
                f.size.width -= 1.f;
                superv.frame = f;
                
                self.font = [UIFont boldSystemFontOfSize:15];
            } else if ([self.text isEqualToString:MultiLanguage(cvcDelete)])
            {
                superv.backgroundColor = [UIColor colorWithRed:46.f/255.f green:46.f/255.f blue:46.f/255.f alpha:1.f];
                [superv setRoundedCorners:(UIRectCornerTopRight | UIRectCornerBottomRight) radius:5.f];
